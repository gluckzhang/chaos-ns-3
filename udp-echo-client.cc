...
int UdpEchoClient::TestConnection(){
  NodeContainer nodes;
  nodes.Create (1);
  //Ipv4AddressHelper address;
  //address.SetBase ("127.1.1.0", "255.255.255.0");
  //UdpEchoClientHelper echoClient (m_peerAddress, m_peerport);
  //echoClient.SetAttribute ("MaxPackets", UintegerValue (nPackets));
  //echoClient.SetAttribute ("Interval", TimeValue (Seconds (1.0)));
  //echoClient.SetAttribute ("PacketSize", UintegerValue (2048));

  TypeId tid = TypeId::LookupByName ("ns3::UdpSocketFactory");
  Ptr<Socket> testsocket = Socket::CreateSocket (nodes.Get(0), tid); 
  //ApplicationContainer clientApps = echoClient.Install (nodes.Get (0));
  //clientApps.Start (Seconds (2.0));
  //clientApps.Stop (Seconds (3.0));
  int value = testsocket->Connect (InetSocketAddress (Ipv4Address::ConvertFrom(m_peerAddress), m_peerPort));
  testsocket->Close();
  return value;
}
...
{ 
  ...
  m_txTrace (p);
  //if(TestConnection()==(-1)){
  //  std::cout << "Connection Failed! destination socket missing" << "\n";
  //}else
  //{
