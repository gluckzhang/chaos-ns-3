...
void ReadLog(){
  ...
  if(success){
    oss << "EXPERIMENT SUCCESS" << endl;
  }else{
    oss << "EXPERIMENT FAILED" << endl;
  }
  infile.close();
}
...
void ProduceRoads(vector<SendEvents*>& events){
    ...
    for( auto elem: events){
      ...
      DoingChaos(elem->attribute);
		  ifstream infile("scratch/caseNetFlixlogs3.txt");
...
void CalculateChaosPaths(vector<SendEvents*>& events){
  	for(auto elem: events){
  	  ...
  	  oss << "[From Node " << elem->start << " To Node " << elem->end << "] POSSIBLE ROADS : " ;
  	  ...
  	  oss << endl;
  	  oss << "[From Node " << elem->start << " To Node " << elem->end << "] CHAOSPATHS : " ;
      ...
      for(auto solution : solutions){
        ...
        oss << "COMMENCING LINAGE FAULTINJECTION " << endl;
        oss << "TESTING CHAOS HYPOTHESIS FOR CHAOS SOLUTION AT NODES : ";
