...
void DoingChaos(string attribute){
   std::string command = "./waf --run \"scratch/caseNetFlixchaosver1" + attribute + " 2> scratch/caseNetFlixlogs.txt";
   exec(command.c_str());
    ...
    for (i=0; i<ChaosExperiments.size(); i++){ 
      if(NextExperiment){
        ...
        clog << "BEGIN CHAOSEXPERIMENT NR " << i << " EXPERIMENTNAME: " << ChaosExperiments[i].name << "\n"; 
		    ...
		    exec("diff scratch/caseNetFlixNonChaoslogs.txt scratch/caseNetFlixlogs.txt | grep '>' | sed 's/^> //g' > scratch/caseNetFlixlogsdiff.txt");
        std::ifstream infile("scratch/caseNetFlixlogsdiff.txt");
