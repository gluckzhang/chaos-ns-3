...
void DoingChaos(string attribute){
   std::string command = "./waf --run \"scratch/second_chaos" + attribute + " 2> scratch/secondlogs.txt";
   exec(command.c_str());
    ...
    for (i=0; i<ChaosExperiments.size(); i++){ 
      if(NextExperiment){
        ...
        clog << "BEGIN CHAOSEXPERIMENT NR " << i << " EXPERIMENTNAME: " << ChaosExperiments[i].name << "\n"; 
		    ...
		    exec("diff scratch/secondlogsnonchaos.txt scratch/secondlogs.txt | grep '>' | sed 's/^> //g' > scratch/secondlogsdiff.txt");
        std::ifstream infile("scratch/secondlogsdiff.txt");
