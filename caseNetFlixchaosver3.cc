...
struct MyNode{
	...
	vector<string> IPs;
	vector<pair<int,int>> SendReceivepairs;
};
...
void SendEventCallback(Ptr<UdpEchoClient> client){
  ...
  ostringstream oss;
  pair<int,int> pr = node->SendReceivepairs[0];
  ...
  string attribute = "--ChaosEvent=" + oss.str() + "\" "; 
  string command = "./waf --run \"scratch/caseNetFlixcontrollerver3 " + attribute + " 2> scratch/caseNetFlixcontrollerlogs3.txt";
  exec(command.c_str());
  exec("diff scratch/caseNetFlixlogs3UnwantedLogs.txt scratch/caseNetFlixcontrollerlogs3.txt | grep '>' | sed 's/^> //g' > scratch/caseNetFlixcontrollerlogs3diff.txt");
  ifstream infile("scratch/caseNetFlixcontrollerlogs3diff.txt");
...
void BuildEchoClient(Ptr<Node> clientnode,Ipv4Address address,int port,double starttime){
  ...
  address.Print(oss);
	...
	pair<int,int> pr(clientnode->GetId(),servernodeid);
	allmynodes[clientnode->GetId()]->SendReceivepairs.push_back(pr);
